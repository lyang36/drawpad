...
public class Bezier {
     ...
     * @param points
     */
    public Bezier(ArrayList<EPointF> points) {
        int n = points.size();
        if (n < 3) {
            // Cannot create bezier with less than 3 points
            return;
        }
        bPoints = new EPointF[2 * (n - 2)];
        double paX, paY;
        ...
        for (int i = 0; i < n - 2; i++) {
            ...
            bPoints[2 * i] = new EPointF((int) p1X, (int) p1Y);
            ...
            bPoints[2 * i + 1] = new EPointF((int) p2X, (int) p2Y);
        }
    }
